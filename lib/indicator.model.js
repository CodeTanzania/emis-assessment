...
const STAGES = getStrings('ASSESSMENT_STAGES', [
  STAGE_BEFORE, STAGE_DURING, STAGE_AFTER
]);
...
const OPTION_AUTOPOPULATE = ({
  select: { subject: 1, topic: 1, stage: 1, phases: 1 },
...
const IndicatorSchema = new Schema({
   ...
   *   "subject": "Water",
   *   "topic": "Water supply before the disaster",
   *   "stage": "Before",
   *   "phases": ["Response", "Recovery"]
   * }
   */
  base: {
    type: ObjectId,
   ...
   * @instance
   * @example
   * Water supply before the disaster
   */
  topic: {
    type: String,
    trim: true,
  ...
  /**
   * @name phases
   * @description Disaster management phase under which indicator is applicable.
   *
   * @type {object}
   * @property {object} type - schema(data) type
  ...
  /**
   * @name stage
   * @description Human readable stage of an indicator(s).
   *
   * @type {object}
   * @property {object} type - schema(data) type
   * @property {boolean} enum - list of acceptable values
...
IndicatorSchema.statics.STAGES = STAGES;
IndicatorSchema.statics.PHASES = PHASES;
  ...
  async.waterfall([
    ...
    function findExistingIndicator(next) {
      ...
      criteria = (
        criteria._id ?
        _.pick(criteria, '_id') :
        _.pick(criteria, 'subject', 'topic', 'stage')
      );
